...
def create_cloud_project(resolve_obj, project_name: str, folder_path: str = None) -> Dict[str, Any]:
    """
        ...
        if result:
            ...
            if new_project:
                return {
                    ...
                    "project_id": new_project.GetUniqueId() if hasattr(new_project, "GetUniqueId") else None,
                }
            else:
                return {
                    "success": True,
            ...
            return {"success": False, "error": f"Failed to create cloud project '{project_name}'"}
...
def import_cloud_project(resolve_obj, cloud_id: str, project_name: str = None) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if project_name:
            ...
            imported_project = project_manager.ImportCloudProject(cloud_id, project_name)
        else:
        ...
        if imported_project:
            return {
                ...
                "project_id": imported_project.GetUniqueId() if hasattr(imported_project, "GetUniqueId") else None,
            }
        else:
            return {"success": False, "error": f"Failed to import cloud project with ID '{cloud_id}'"}
...
def restore_cloud_project(resolve_obj, cloud_id: str, project_name: str = None) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if project_name:
            ...
            restored_project = project_manager.RestoreCloudProject(cloud_id, project_name)
        else:
        ...
        if restored_project:
            return {
                ...
                "project_id": restored_project.GetUniqueId() if hasattr(restored_project, "GetUniqueId") else None,
            }
        else:
            return {"success": False, "error": f"Failed to restore cloud project with ID '{cloud_id}'"}
...
def get_cloud_project_list(resolve_obj) -> Dict[str, Any]:
    ...
    try:
        ...
        if cloud_projects is not None:
            ...
            return {"success": False, "error": "Failed to get cloud project list or none available"}

    except Exception as e:
        logger.error(f"Error getting cloud project list: {str(e)}")
        return {"success": False, "error": f"Error getting cloud project list: {str(e)}"}
...
def export_project_to_cloud(resolve_obj, project_name: str = None) -> Dict[str, Any]:
    ...
    try:
        ...
        # Check if method exists in the API
        if not hasattr(project, "ExportToCloud") and not hasattr(project_manager, "ExportProjectToCloud"):
            return {"success": False, "error": "Cloud export methods not available in this version of DaVinci Resolve"}
        ...
        if export_result:
            ...
            return {"success": False, "error": f"Failed to export project '{project.GetName()}' to cloud"}

    except Exception as e:
        logger.error(f"Error exporting project to cloud: {str(e)}")
        return {"success": False, "error": f"Error exporting project to cloud: {str(e)}"}


def add_user_to_cloud_project(resolve_obj, cloud_id: str, user_email: str, permissions: str = "viewer") -> Dict[str, Any]:
    """
    ...
    try:
        ...
        result = project_manager.AddUserToCloudProject(cloud_id, user_email, permission_value)
        ...
        if result:
            ...
            return {"success": False, "error": f"Failed to add user '{user_email}' to cloud project"}

    except Exception as e:
        logger.error(f"Error adding user to cloud project: {str(e)}")
        return {"success": False, "error": f"Error adding user to cloud project: {str(e)}"}


def remove_user_from_cloud_project(resolve_obj, cloud_id: str, user_email: str) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if result:
            return {"success": True, "message": f"Successfully removed user '{user_email}' from cloud project"}
        else:
            return {"success": False, "error": f"Failed to remove user '{user_email}' from cloud project"}

    except Exception as e:
        logger.error(f"Error removing user from cloud project: {str(e)}")
        return {"success": False, "error": f"Error removing user from cloud project: {str(e)}"}
