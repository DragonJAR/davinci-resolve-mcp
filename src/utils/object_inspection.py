...
def get_lua_table_keys(lua_table: Any) -> List[str]:
    ...
    if hasattr(lua_table, "GetKeyList"):
        try:
            ...
            logger.debug(f"GetKeyList failed: {e}")
            pass
    ...
    try:
        ...
        logger.debug(f"Lua table iteration failed: {e}")
        pass
        ...
        if keys:
            ...
            for key in keys:
                try:
                    ...
                    logger.debug(f"Failed to get value for key '{key}': {e}")
                    result[key] = None
            return result
        ...
        try:
            ...
            while True:
                try:
                    ...
                    logger.debug(f"List conversion failed: {e}")
                    break
