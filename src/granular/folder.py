...
def get_resolve_version() -> Dict[str, Any]:
    ...
    if resolve is None:
        return {"error": "Not connected to DaVinci Resolve"}
    return {"product": resolve.GetProductName(), "version": resolve.GetVersion(), "version_string": resolve.GetVersionString()}
...
def get_folder_clip_list(folder_path: str = "") -> Dict[str, Any]:
    ...
    if clips:
        return {
            ...
            "clips": [{"name": c.GetName(), "unique_id": c.GetUniqueId()} for c in clips],
        }
...
def get_folder_subfolder_list(folder_path: str = "") -> Dict[str, Any]:
    ...
    if subs:
        return {
            "folder": folder.GetName(),
            "subfolders": [{"name": s.GetName(), "unique_id": s.GetUniqueId()} for s in subs],
        }
