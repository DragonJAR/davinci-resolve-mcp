...
@mcp.tool()
def import_media(params: Dict[str, Any]) -> Dict[str, Any]:
    """Import media into the current project's media pool.

    Args:
        params: Dictionary with 'paths' (list of file paths) or 'clip_infos' (list of clip info dicts)
    """
    pm, current_project = get_current_project()
    if not current_project:
        return {"error": "No project currently open"}
    ...
    clip_infos = params.get("clip_infos", [])
    if clip_infos:
...
def add_clip_mattes_to_media_pool(
    media_pool_item_id: str, matte_paths: List[str]
) -> Dict[str, Any]:
...
@mcp.tool()
def start_rendering_jobs(
    job_ids: Optional[List[str]] = None, is_interactive_mode: bool = False
) -> Dict[str, Any]:
...
def delete_media_pool_folders(folder_names: List[str]) -> Dict[str, Any]:
    ...
    folders = [
        sub
        for sub in (current.GetSubFolderList() or [])
        if sub.GetName() in folder_names
    ]
    if not folders:
...
def move_clips_to_folder(
    clip_ids: List[str], target_folder_path: str
) -> Dict[str, Any]:
...
@mcp.tool()
def move_media_pool_folders(
    folder_names: List[str], target_folder_path: str
) -> Dict[str, Any]:
...
def get_folder_clip_list(folder_path: str = "") -> Dict[str, Any]:
    ...
    if clips:
        return {
            ...
            "clips": [
                {"name": c.GetName(), "unique_id": c.GetUniqueId()} for c in clips
            ],
        }
...
def get_folder_subfolder_list(folder_path: str = "") -> Dict[str, Any]:
    ...
    if subs:
        return {
            "folder": folder.GetName(),
            "subfolders": [
                {"name": s.GetName(), "unique_id": s.GetUniqueId()} for s in subs
            ],
        }
...
def export_stills_from_album(
    album_index: int, folder_path: str, file_prefix: str = "still", format: str = "dpx"
) -> Dict[str, Any]:
...
def delete_stills_from_album(
    album_index: int, still_indices: List[int]
) -> Dict[str, Any]:
...
@mcp.tool()
def graph_apply_grade_from_drx(
    drx_path: str,
    grade_mode: int = 0,
    item_index: int = 0,
    track_type: str = "video",
